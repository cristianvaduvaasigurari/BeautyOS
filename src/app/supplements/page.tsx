...
export default function SupplementsPage() {
  const articles = Object.values(editorialContent).filter(a => a.category === "Supplements");
    ...
    <div className="min-h-screen pt-24 pb-32 px-6 max-w-[1440px] mx-auto animate-fadeIn">
      <div className="max-w-3xl space-y-6">
        ...
        <p className="text-lg text-zinc-400 font-normal leading-relaxed max-w-2xl">
          Protein, Creatine, Omega-3, and Longevity activators.
        </p>
      </div>

      <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8">
        {articles.map((article, index) => (
          <Link key={article.id} href={`/supplements/${article.slug}`} className="p-8 rounded-[32px] bg-gradient-to-br from-[#101114] to-[#17181B] border border-white/[0.05] hover:border-white/[0.15] transition-all group cursor-pointer block">
